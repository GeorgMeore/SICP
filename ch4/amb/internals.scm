...
(define (remove-binding! var frame)
  (cond ((null? (cdr frame))
          (error "Trying to remove nonexistent binding" var))
        ((eq? (caar frame) var)
...
(define (variable-is-defined? var env)
  (not (null? (get-binding var (first-frame env)))))

(define (undefine-variable! var env)
  (remove-binding! var (first-frame env)))
