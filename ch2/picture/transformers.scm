...
(define (rotate painter deg)
  ...
  (define (normalize x n)
    (- x (* n (floor (/ x n)))))
  (define (x deg)
    (let ((rad (degrees->radians (normalize deg 90))))
      (/ (+ (tan (- rad pi/4)) 1) 2)))
  (define (rotate-frame frame deg)
    (let ((deg (normalize deg 360)) (x (x deg)))
      (cond ((< deg 90)
...
(define (corner-split painter n)
  (if (> n 0)
      (let ((rest (corner-split painter (- n 1))))
        (beside (below rest painter)
                (below rest rest)))
      painter))

(define (square-of-four tl tr bl br)
  (lambda (painter)
    (below (beside (tl painter) (tr painter))
           (beside (bl painter) (br painter)))))

(define (row . painters)
  (define (n-row painters n)
    (if (> n 1)
        (beside-scaled (car painters)
